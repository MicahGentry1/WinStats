...
do
{
    if (!runOnce && !Console.IsOutputRedirected)
    {
        Console.Clear();
    }
...
SystemInfo CollectStats()
{
    ...
    if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
    {
        ...
        {
            memoryErrorCode = Marshal.GetLastWin32Error();
        }
...
class SystemInfo
{
    ...
    public int? MemoryErrorCode { get; set; }
    ...
    public void Print()
    {
        ...
        else if (MemoryErrorCode is int errorCode)
            Console.WriteLine($"Memory:  N/A (Win32 error {errorCode})");
        else
